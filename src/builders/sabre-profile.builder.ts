...
export class SabreProfileBuilder {
  ...
  buildUpdateRequest(
    ...
    return {
      Sabre_OTA_ProfileUpdateRQ: {
        ...
        ProfileInfo: {
          Profile: this.buildProfileContentForUpdate(profile, currentProfile),
        },
      },
    };
  }
  ...
  buildDeleteRequest(
    profileId: string,
    domainId: string,
    clientCode: string = "TN",
    clientContext: string = "TMP",
  ): any {
    return {
  ...
  private buildProfileContentForUpdate(profile: any, currentProfile: any) {
    const now = new Date().toISOString();
    const updateTime = currentProfile.updated
      ? currentProfile.updated.toISOString()
      : now;
    const content: any = {
      $: {
        CreateDateTime: currentProfile.created
          ? currentProfile.created.toISOString()
          : now,
        UpdateDateTime: updateTime,
      },
      TPA_Identity: this.buildIdentity(profile),
    ...
    // Always include Traveler for both personal and business profiles
    content["Traveler"] = this.buildTraveler(profile);

    // Add RemarkInfo if there are any remarks
    if (profile.remarks && profile.remarks.length > 0) {
      content["RemarkInfo"] = this.buildRemarks(profile.remarks);
    }
  ...
  private buildIdentity(profile: CanonicalProfile): any {
    return {
      $: {
        ...
        ClientContextCode:
          profile.metadata.customFields?.clientContext || "TMP",
  ...
  private buildTraveler(profile: CanonicalProfile): any {
    ...
    if (profile.employment) {
      traveler["Customer"]["EmploymentInfo"] = this.buildEmployment(
        profile.employment,
      );
    }
  ...
  private buildPreferences(prefs: TravelPreferences): any {
    return {
      AirlinePref: prefs.airlines.map((a) => ({
        ...
        SSR_Pref: (a.specialService || []).map((ssr) => ({
          $: { SSR_Code: ssr },
        })),
      })),
      HotelPref: prefs.hotels.map((h) => ({
        $: {
          ...
          RoomType: h.roomType,
          SmokingAllowed: h.smokingPreference
            ? this.mapSmokingPreference(h.smokingPreference)
            : undefined,
          BedType: h.bedType,
        },
      })),
      VehicleRentalPref: prefs.cars.map((c) => ({
        $: {
          ...
          TransmissionType: c.transmission
            ? this.mapTransmissionType(c.transmission)
            : undefined,
        },
      })),
    };
  ...
  */
  ...
  private mapAddressType(type: string): string {
    if (type.toUpperCase().includes("HOME")) return "H";
    if (
      type.toUpperCase().includes("WORK") ||
      type.toUpperCase().includes("BUS")
    )
      return "B";
    return "H";
  }
